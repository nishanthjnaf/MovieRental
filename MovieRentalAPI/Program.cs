// KEEP your existing usings. You do NOT need Microsoft.OpenApi.* at all now.
using AspNetCoreRateLimit;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Microsoft.AspNetCore.Builder;
using Microsoft.EntityFrameworkCore;
using Microsoft.IdentityModel.Tokens;
// using Microsoft.OpenApi;                // <- not required anymore
using MovieRentalAPI.Interfaces;
using MovieRentalAPI.Models;
using MovieRentalAPI.Repositories;
using MovieRentalAPI.Services;
using MovieRentalModels;
using NSwag;                                // <— NSwag types
using NSwag.Generation.Processors.Security; // <— Security processors
using System.Text;
...
{
    // Explicit document name so the path becomes /swagger/v1/swagger.json
...
{
    // Serve the JSON at a fixed path
    app.UseOpenApi(cfg =>
    {
    ...
    // Point the UI explicitly to that JSON
    app.UseSwaggerUi(cfg =>
    {
        cfg.Path = "/swagger"; // UI at /swagger
